...
for f_name in file_list :
    ...
    plt.xlabel('Star ID', fontsize=14)
    plt.ylabel('msky +- sky_std', fontsize=14)
    plt.xticks(np.arange(0, N_stars, step=int((N_stars+100)/100)*10))
    plt.grid(ls=':')
    plt.savefig(f_name[:-4]+'_Annulus_sky_result.png', overwrite=True)
    #plt.show()
    
    #%%
    # change img value to 16 bit Integer in ADU
    img = np.array(img*65536.0, dtype=np.uint16)
    ronoise = 8.00  # electrons
    ...
    plt.xlabel('Star ID', fontsize=14)
    plt.ylabel('Instrumental mag', fontsize=14)
    plt.xticks(np.arange(0, N_stars, step=int((N_stars+100)/100)*10))
    plt.grid(ls=':')
    plt.savefig(f_name[:-4]+'_Annulus_AP_result.png', overwrite=True)
    #plt.show()
    
    #print and save result
    print(apert_result)
    with open(f_name[:-4]+'_Annulus_AP_result.csv', 'w') as f:
