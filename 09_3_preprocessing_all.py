...
f_name_bias = 'bias-median.fit'
f_name_dark0 = 'dark0-median.fit'
f_name_dark = 'dark-median.fit'
f_name_flat0 = 'flat0-median.fit'
f_name_flat = 'flat-median.fit'
#%%
bias_list = sorted(glob(os.path.join(dir_name, 'bias*.fit')))
dark_list = sorted(glob(os.path.join(dir_name, 'dark*.fit')))
...
for chl in[''] :
    ...
    file_list = sorted(glob(os.path.join(dir_name, 'g*'+chl+'.fit')))
    for objname in file_list :
        ...
        reduced.write(objname[:-4]+'_p.fit', overwrite =True)