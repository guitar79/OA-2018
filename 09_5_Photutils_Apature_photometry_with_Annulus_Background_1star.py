...
for f_name in file_list[0:2]:
    ...
    #img[img < 1.e-6] = 1.e-6
    ####################################
    #FWHM   = 2.0    # for qsi image
    #sky_th = 100    # for qsi image
    ...
    if len(DAOfound)==0 :
        ...
        apert_result = 'ID, Msky, sky_std, Sky count Pixel_N, Sky reject Pixel_N, flux_star, flux_err, mag_ann, merr_ann,ronoise, gain\n'
        ...
        for star_ID in range(0, N_stars):
            ...
            apert_result += '{0:04}, {1:.5f}, {2:.5f}, {3:4d}, {4:3d}, {5:.5f}, {6:.5f}, {7:.3f}, {8:.3f}, {9:.3f}, {10:.3f}\n'\
            .format(star_ID, msky, sky_std, nsky, nrej, flux_star, flux_err, mag_ann[star_ID], merr_ann[star_ID], ronoise, gain)
            ...
            plt.gcf().suptitle('DAOstarfinder Annulus result: {0!s} \n \
                   ...
                   star ID: {5:04},  read out noise: {6:.3f}, gain: {7:.3f}\n \
